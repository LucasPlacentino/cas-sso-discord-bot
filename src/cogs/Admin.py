...
class Admin(commands.Cog):
    ...
    @commands.slash_command(
        name="update",
        description="Force the database and servers update.",
    ...
    @commands.slash_command(
        name="yearly-update",
        description="Remove all members roles and send them an email about it.",
        guilds=[int(os.getenv("ADMIN_GUILD_ID"))],
        default_member_permissions=disnake.Permissions.all(),
        dm_permission=False,
    )
    async def yearly_update(
        ...
        raison: str = commands.Param(
            description="""Update reason (default: "Annual verification")""",
            default="Annual verification",
        ),
    ):
    ...
    @user.sub_command(name="add", description="Add a Discord user.")
    async def user_set(
        self,
        inter: disnake.ApplicationCommandInteraction,
        username: str = commands.Param(description="The user to add."),
    ):
        user = next((user for user in self.bot.users if f"{user.name}#{user.discriminator}" == username))
        if not user:
            await inter.response.send_message(
                embed=disnake.Embed(
                    description=f"No Discord user with username: {username}.",
        ...
        if user in Database.ulb_users.keys():
            await inter.response.send_message(
                embed=disnake.Embed(
                    description=f"The user {username} is already in the database. Use **/`user edit`** if you want to modify them.",
    ...
    @user.sub_command(name="edit", description="Edit the servers user (only one parameter required).")
    async def user_edit(
        self,
        inter: disnake.ApplicationCommandInteraction,
        user_id: str = commands.Param(
            description="The Discord ID of the user to edit.", min_length=17, max_length=20, default=None
        ),
        name: str = commands.Param(description="The name of the user to edit.", default=None),
        username: str = commands.Param(description="The Discord username", default=None),
        email: str = commands.Param(description="The user email to edit.", default=None),
    ):
        if user_id:
            ...
            if user == None:
                await inter.response.send_message(
                    embed=disnake.Embed(
                        title="User info",
                        description=f"The ID corresponds to no known Discord user.",
                        color=disnake.Colour.orange(),
                    ),
                    ephemeral=True,
                )
            ...
            if user not in Database.ulb_users.keys():
                await inter.response.send_message(
                    embed=disnake.Embed(
                        title="User info",
                        description=f"The ID corresponds to no known Servers user.",
                        color=disnake.Colour.orange(),
                    ),
                    ephemeral=True,
                )
            ...
            if user == None:
                await inter.response.send_message(
                    embed=disnake.Embed(
                        title="User info",
                        description=f"The name corresponds to no known user.",
                        color=disnake.Colour.orange(),
                    ),
                    ephemeral=True,
                )
            ...
            if user == None:
                await inter.response.send_message(
                    embed=disnake.Embed(
                        title="User info",
                        description=f"The email corresponds to no known user.",
                        color=disnake.Colour.orange(),
                    ),
                    ephemeral=True,
                )
            ...
            await inter.response.send_message(
                embed=disnake.Embed(
                    title="User info",
                    description=f"Specify the ID, name or email in the command.",
                    color=disnake.Colour.orange(),
                ),
                ephemeral=True,
            )
    ...
    @user.sub_command(
        name="info", description="See the registered user info (only one parameter required)."
    )
    async def user_info(
        self,
        inter: disnake.ApplicationCommandInteraction,
        name: str = commands.Param(description="The name of the user", default=None),
        username: str = commands.Param(description="The Discord username of the user", default=None),
        email: str = commands.Param(description="The email of the user.", default=None),
        user_id: str = commands.Param(
