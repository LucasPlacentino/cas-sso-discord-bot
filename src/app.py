...
from typing import Any, Literal, Optional, List, Annotated # TODO: use Annotated as much as possible
import sys
import signal
...
from fastapi_discord.exceptions import ScopeMissing as DiscordOAuthScopeMissing
...
DEBUG: bool=True if getenv("DEBUG") or getenv("DEBUG") is not None or getenv("DEBUG") != "" else False
DEV_ENV: bool=True if getenv("DEV_ENV") or getenv("DEV_ENV") is not None or getenv("DEV_ENV") != "" else False
if DEV_ENV:
...
# ! does this even work ?
LangCodePath = Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en", "fr"])] # TODO: use this to improve endpoints def readability ? Not sure if this works with FastAPI

# TODO: add function to parse lang from session or headers in utils.py ?
...
DISCORD_TOKEN_URL: str = "https://discord.com/api/v10/oauth2/token" # ? https://github.com/Tert0/fastapi-discord/issues/96
...
class App(FastAPI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.locale: Locale = None # type: ignore # extends FastAPI with locale, initializes during startup in lifespan()
        self.discord: DiscordOAuthClient = discord_auth
        self.cas_attr_filter = None # initializes during init()

@asynccontextmanager
async def lifespan(app: App): # replaces deprecated @app.on_event("startup") and @app.on_event("shutdown")
    # --- startup ---
...
# CAS Client
cas_client = CASClient( # Use CASClientV2 or V3 ? for proxy_callback url (for pgt from pgtiou) see docs: https://djangocas.dev/docs/latest/_modules/cas.html#CASClient
...
CAS_VALIDATE_PATH: str = getenv('CAS_VALIDATE_PATH', "/serviceValidate") # or "/proxyValidate" ?

# Session Middleware for FastAPI
def _get_app_secret_key() -> str:
    _app_secret_key: str | None = getenv('APP_SECRET_KEY') if DEV_ENV else secrets.token_urlsafe(64) # generate a random secret key (if not DEV_ENV and not set in env var)
...
def env_var(key: str, default: Optional[str] = None):
    value: str | None = getenv(key, default)
    if value is None:
...
async def index_without_lang(request: Request) -> RedirectResponse:
...
@app.get('/{lang}/')
async def index(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> None | RedirectResponse | _TemplateResponse: # -> None for favicon fix
...
async def profile_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
...
async def profile(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> RedirectResponse:
    request.session['lang'] = lang
    return RedirectResponse(url=f"/{lang}/user")
@app.get('/me', response_class=RedirectResponse)
async def me_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
    return RedirectResponse(url=f"/{lang}/user", status_code=status.HTTP_308_PERMANENT_REDIRECT)
@app.get('/{lang}/me', response_class=RedirectResponse)
async def me(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> RedirectResponse:
    request.session['lang'] = lang
    return RedirectResponse(url=f"/{lang}/user")


@app.get('/user', response_class=RedirectResponse)
async def user_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
    return RedirectResponse(url=f"/{lang}/user", status_code=status.HTTP_308_PERMANENT_REDIRECT)

@app.get('/{lang}/user')
async def user(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])], debug: Annotated[Optional[str], Query(None, description="debug mode key")], discorddebug: Annotated[Optional[bool], Query(None, description="Discord debug mode")]) -> _TemplateResponse | HTMLResponse | RedirectResponse:
    request.session['lang'] = lang
    if DEBUG:
    ...
    if user:
        ...
        # %%%%%%%%%%%%% user is not Discord authenticated %%%%%%%%%%%%%
            ...
            if DEBUG:
                cas_user = str(user['user']) # ! TODO: sure about this ?
                logout_url: URL = request.url_for('logout')
    ...
    if DEBUG:
        login_url: URL = request.url_for('login')
...
async def login(request: Request, ticket: Annotated[Optional[str], Query(None, description="CAS ticket from CAS server")], next: Annotated[Optional[str], Query(None, description="next path to redirect at the end")]) -> RedirectResponse | HTMLResponse: #TODO: remove next ?
    service_ticket: str | None = ticket # ST from user to verify with CAS server
    ...
    if not service_ticket: # first login -> redirect to CAS
        ...
        cas_login_url: URL = URL(cas_client.get_login_url()) # ! is wrapping in URL() really needed ?
        if DEBUG:
    ...
    user_from_cas, attributes_from_cas, pgtiou = cas_client.verify_ticket(service_ticket) # type: ignore # pgtIou means Proxy Granting Ticket IOU
    ##*##############
    ...
    if isinstance(attributes_from_cas, dict): # keeps type checking happy
        filtered_attributes = {key: value for key, value in attributes_from_cas.items() if key in app.cas_attr_filter}
    else:
        logger.error(f"attributes_from_cas is not a dict: {attributes_from_cas}")
        filtered_attributes: dict = {}
        attributes_from_cas: dict = {}
    ...
    if not user_from_cas: # Failed to verify service_ticket
        ...
        request.session['user'] = dict(user=user_from_cas) # ! TODO: is this correct ?
        return response


@app.get('/logout', response_class=RedirectResponse)
async def logout(request: Request) -> RedirectResponse:
    # clear session cookie is done on logout callback
    user = request.session.get("user")
    if user:
    # if request.session.get("user") is not None:
        redirect_url: URL = request.url_for('logout_callback')
        cas_logout_url: URL = URL(cas_client.get_logout_url(redirect_url)) # ! is wrapping in URL() really needed ?
        if DEBUG:
...
async def discord_login(request: Request) -> RedirectResponse:
    user = request.session.get("user")
    if DEBUG or user:
    # if DEBUG or (request.session.get("user") is not None):
        # check if already logged in with discord, redirect to user if so
        if request.session.get("discord_token") is not None:
            return RedirectResponse(request.url_for('user',lang=request.session['lang']))
        ...
        #request.session['discord_oauth_state'] = "my_test_state"
...
#TODO: NOT USE THIS ?
async def get_user(token: str = Depends(discord_auth.get_token)) -> DiscordUser:
    if "identify" not in discord_auth.scopes:
        raise DiscordOAuthScopeMissing("identify")
    route = "/users/@me"
    return DiscordUser(**(await discord_auth.request(route, token)))
    #return user
#TODO: NOT USE THIS ?
async def get_user_guilds(token: str = Depends(discord_auth.get_token)) -> List[DiscordGuild]:
    if "guilds" not in discord_auth.scopes:
        raise DiscordOAuthScopeMissing("guilds")
...
async def discord_callback(request: Request,
    ...
    # if DEBUG or (request.session.get("user") is not None):
        ...
        try:
            ...
            assert state == request.session.get("discord_oauth_state")
        except AssertionError:
            logger.error("discord_callback: state does not match")
            request.session.clear() # ! overkill ?
            return RedirectResponse(request.url_for('login'), status_code=status.HTTP_406_NOT_ACCEPTABLE)

        request.session.pop("discord_oauth_state") # remove state from session
        ...
        try:
            ...
            if e is DiscordOAuthScopeMissing:
                logger.error(f"DiscordOAuthScopeMissing error in Discord API Client: missing \"guilds\" in scopes -> ignoring user guilds")
            else:
                logger.error(f"discord_callback: error getting user guilds: {e} -> ignoring user guilds")

        lang: str = get_preferred_lang(request, app.locale.lang_list)
        return RedirectResponse(request.url_for('user', lang=lang))
        ##try:
...
async def isDiscordAuthenticated(request: Request) -> bool:
    try:
        auth: bool = await discord_auth.isAuthenticated(token=request.session['discord_token'])
        return auth
    except Unauthorized:
        return False
...
async def discord_logout(request: Request) -> RedirectResponse:#, token: str = Depends(discord_auth.get_token)):
    try:
        ...
        if await discord_auth.isAuthenticated(request.session['discord_token']):
            ...
            if DEBUG:
                logger.debug("discord_logout: revoking user discord_token")
            await revoke_discord_token(request.session['discord_token'], "access_token", request.session['discord_username'])
            if DEBUG:
                logger.debug("discord_logout: revoking user discord_refresh_token")
            await revoke_discord_token(request.session['discord_refresh_token'], "refresh_token", request.session['discord_username'])

            if DEBUG:
                logger.debug("discord_logout: revoking user access_token using app token")
            success: bool = await revoke_discord_token(discord_auth.get_token(request), "access_token", request.session['discord_username'])
            ...
            if success:
                ...
                if DEBUG:
                    logger.debug("Removed discord session data")
            else:
                logger.error("Could not revoke user token")
    ...
    #! huh ? :
        ...
        # TODO: use utils function to get preferred lang
        lang: str = get_preferred_lang(request, app.locale.lang_list)
        return RedirectResponse(url=f"/{lang}/user")

# FIXME: #! doesn't seem to be working ?
async def revoke_discord_token(token: str, token_type: Literal["access_token", "refresh_token"], user: str | None = None) -> bool:
...
# ! TODO: rate limit
@app.post('/user/force-add-roles') # POST request # ! what Response type should be here ?
async def force_add_roles(request: Request):
    ...
    """
    lang: str = get_preferred_lang(request, app.locale.lang_list)
    return RedirectResponse(url=f"/{lang}/user", status_code=status.HTTP_303_SEE_OTHER)
    return RedirectResponse(request.url_for('user', lang=lang), status_code=status.HTTP_303_SEE_OTHER)

# ---- other pages ----

@app.get('/help', response_class=RedirectResponse)
async def help_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
    # TODO: Which ? :
...
async def help(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> _TemplateResponse:
    request.session['lang'] = lang
...
async def about_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
...
async def about(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> _TemplateResponse:
    request.session['lang'] = lang
...
async def privacy_policy_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
...
async def privacy_policy(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> _TemplateResponse:
    request.session['lang'] = lang
...
async def terms_of_service_without_lang(request: Request) -> RedirectResponse:
    lang: str = get_preferred_lang(request, app.locale.lang_list)
...
async def terms_of_service(request: Request, lang: Annotated[str, Path(title="2-letter language code", max_length=2, min_length=2, examples=["en","fr"])]) -> _TemplateResponse:
    request.session['lang'] = lang
...
async def unauthorized_error_handler(request: Request):
    error = "Unauthorized"
    return HTMLResponse(templates.TemplateResponse(name="401.html", context={"request": request,"error": error, "current_lang": DEFAULT_LANG}, status_code=401))


@app.exception_handler(RateLimited)
async def rate_limit_error_handler(request: Request, e: RateLimited):
    return HTMLResponse(templates.TemplateResponse(name="429.html", context={"request": request,"retry_after": e.retry_after, "current_lang": DEFAULT_LANG}, status_code=429))
...
async def run_bot() -> None:
    ...
    botRootLogger: logging.Logger = logging.getLogger("bot")
    botRootLogger.setLevel(logging.DEBUG) # if DEBUG else logging.TRACE ?

    consoleHandler = logging.StreamHandler()
    consoleHandler.setFormatter(botLogFormatter)
    consoleHandler.setLevel(logging.TRACE) # type: ignore logging level TRACE added above
    botRootLogger.addHandler(consoleHandler)

    if platform.system() == "Linux":
        fileInfoHandler = logging.handlers.RotatingFileHandler( # type: ignore
            filename="logs/info.log", mode="w", encoding="UTF-8", delay=True, backupCount=5
        )
        fileDebugHandler = logging.handlers.RotatingFileHandler( # type: ignore
            filename="logs/debug.log", mode="w", encoding="UTF-8", delay=True, backupCount=5
        )
        fileInfoHandler.setFormatter(botLogFormatter)
        fileInfoHandler.setLevel(logging.TRACE) # type: ignore logging level TRACE added above
    ...
    # TODO: implement bot
    BOT_TOKEN: str | None = getenv("DISCORD_BOT_TOKEN", None)
    #if not BOT_TOKEN:
...
async def shutdown_signal_listener(task_group) -> None:
    """
...
if __name__ == '__main__':
    ...
    #! TODO: DOES IT REALLY HANDLES EXCEPTIONS AND KEYBOARD INTERRUPTS CORRECTLY ?
    try:
