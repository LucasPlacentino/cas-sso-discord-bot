...
async def index():
    ...
    return await 'Hello! <a href="/login">CAS Login</a>'
...
async def user(method=['GET']):
    if 'username' in session:
        ...
        if not discordAuth.authorized:
            ...
            return await 'Logged in as %s. <a href="/logout">Logout</a>' % session['username']
        else:
    ...
    return await 'Login required. <a href="/login">Login</a>', 403
...
async def login():
    if 'username' in session:
        # Already logged in
        return await redirect(url_for('user'))
    ...
    if not ticket:
        ...
        return await redirect(cas_login_url)
    ...
    if not CASUser:
        return await 'Failed to verify ticket. <a href="/login">Login</a>'
...
async def discord_callback():
    ...
    await link_accounts(session['username'], discordUser.id)
    # Give role to Discord user
    await give_role(discordUser.id)
...
async def redirect_unauthorized(e):
    app.logger.info("Unauthorized request: " + str(e))
